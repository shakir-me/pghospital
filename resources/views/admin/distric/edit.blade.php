          ...
          <div class="col-md-12">
            ...
            <div class="card">
              ...
              @endif
                ...
                <div class="card-body">
       <form action="{{route('distric.update',$edit_data->id)}}" method="post" class="forms-sample"  enctype="multipart/form-data">
                    @csrf
                  <div class="form-row">
...
@endphp
                        ...
                        <select class="form-control" name="division_id">
                        	@foreach ($division as $row)
	                         <option value="{{ $row->id }}" {{ $row->id == $edit_data->division_id ? 'selected' : '' }}>{{ $row->name }}</option>
	                         @endforeach
                        </select>
                      </div>
                        ...
                        <select class="form-control" name="status" >
                        	<option value="1" {{($edit_data->status=="1")?"selected":""}}>Distric Public</option>
                        	<option value="0" {{($edit_data->status=="0")?"selected":""}}>Distric Private</option>
                        </select>
                      </div>
                       </div>
